...
def token_detail(
    ...
    if latest is None:
        ...
        score_input = {**base, "holder_growth_pct": None, "volume_ratio": None}
    else:
        base = {
            "chain": chain,
            "address": address,
        ...
        volume_ratio = None
        if previous and previous["holder_count"] and latest["holder_count"]:
            prev_h = previous["holder_count"]
            curr_h = latest["holder_count"]
            if prev_h > 0:
                holder_growth_pct = (curr_h - prev_h) / prev_h * 100
        if previous and previous["volume_usd"] and latest["volume_usd"]:
            prev_v = previous["volume_usd"]
            curr_v = latest["volume_usd"]
            if prev_v > 0:
                volume_ratio = curr_v / prev_v
    ...
    score = accumulation.calculate(
        ...
        volume_ratio=score_input.get("volume_ratio"),
